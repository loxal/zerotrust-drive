...
Every concurrently open file is still buffered in RAM. Their aggregate plaintext plus a transient
ciphertext buffer while flushing must remain below available memory. Reads validate the exact
ciphertext size from the authenticated index before allocating and decrypt in place, avoiding a
second file-sized plaintext allocation during open. A true bounded-memory implementation requires
a versioned, authenticated chunk format rather than plaintext temporary files.
...
unexpected staging entries; or a stale temp from an interrupted write.
...
reducing unnecessary cloud uploads. The encrypted index remains one authenticated object and is
rewritten whenever changed metadata is committed.
...
Normal v1 content commits still replace dirty blobs before replacing the encrypted index. A local
crash in that interval can leave the last index and blobs from different generations. The
sibling/temp detector preserves evidence but cannot automatically recover a crash that happened
before an index temp existed. A staged normal-write manifest and fault-injected recovery suite are
required before claiming local multi-file failure atomicity.

A brand-new store publishes `_kdf.json` before its first encrypted index. If initialization stops
...
Disk filenames begin with a 6-digit hex counter (`000001.age` through `ffffff.age`) and grow
...
entries** and a 64 MiB manifest. Live encrypted indexes are capped at 64 MiB before decryption.
